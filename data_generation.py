...
def generate_first_k(master_path, k):
    dataset_path = "movielens/ml-1m"
    rate_list = load_list("{}/m_rate.txt".format(dataset_path))
    genre_list = load_list("{}/m_genre.txt".format(dataset_path))
    actor_list = load_list("{}/m_actor.txt".format(dataset_path))
    ...
    for state in states:
        ...
        for _, user_id in tqdm(enumerate(dataset.keys())):
            ...
            support_x_app = None
            for m_id in tmp_x[indices[:min(k, len(indices)-10)]]:
                m_id = int(m_id)
                tmp_x_converted = torch.cat((movie_dict[m_id], user_dict[u_id]), 1)
                try:
                    support_x_app = torch.cat((support_x_app, tmp_x_converted), 0)
            ...
            for m_id in tmp_x[indices[-10:]]:
                ...
                try:
                    ...
                    query_x_app = tmp_x_converted
            support_y_app = torch.FloatTensor(tmp_y[indices[:min(k, len(indices)-10)]])
            query_y_app = torch.FloatTensor(tmp_y[indices[-10:]])

            pickle.dump(support_x_app, open("{}/{}/supp_x_first_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
            pickle.dump(support_y_app, open("{}/{}/supp_y_first_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
            pickle.dump(query_x_app, open("{}/{}/query_x_first_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
            pickle.dump(query_y_app, open("{}/{}/query_y_first_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
            with open("{}/log/{}/supp_x_first_{}_{}_u_m_ids.txt".format(master_path, state, k, idx), "w") as f:
                for m_id in tmp_x[indices[:min(k, len(indices)-10)]]:
                    f.write("{}\t{}\n".format(u_id, m_id))
            with open("{}/log/{}/query_x_first_{}_{}_u_m_ids.txt".format(master_path, state, k, idx), "w") as f:
                for m_id in tmp_x[indices[-10:]]:
                    f.write("{}\t{}\n".format(u_id, m_id))
            idx += 1


def generate_batch_k(master_path, k):
    dataset_path = "movielens/ml-1m"
    rate_list = load_list("{}/m_rate.txt".format(dataset_path))
    genre_list = load_list("{}/m_genre.txt".format(dataset_path))
    actor_list = load_list("{}/m_actor.txt".format(dataset_path))
    ...
    if not os.path.exists("{}/warm_state/batch/".format(master_path)):
        for state in states:
            os.mkdir("{}/{}/batch/".format(master_path, state))
    ...
    for state in states:
        ...
        for _, user_id in tqdm(enumerate(dataset.keys())):
            ...
            for batch in range(seen_movie_len // (2*k)):
                support_x_app = None
                for m_id in tmp_x[indices[batch*2*k:batch*2*k+k]]:
                    m_id = int(m_id)
                    tmp_x_converted = torch.cat((movie_dict[m_id], user_dict[u_id]), 1)
                    try:
                        support_x_app = torch.cat((support_x_app, tmp_x_converted), 0)
                ...
                query_x_app = None
                for m_id in tmp_x[indices[batch*2*k+k:batch*2*k+2*k]]:
                    m_id = int(m_id)
                    u_id = int(user_id)
                    tmp_x_converted = torch.cat((movie_dict[m_id], user_dict[u_id]), 1)
                    try:
                        ...
                        query_x_app = tmp_x_converted
                support_y_app = torch.FloatTensor(tmp_y[indices[batch*2*k:batch*2*k+k]])
                query_y_app = torch.FloatTensor(tmp_y[indices[batch*2*k+k:batch*2*k+2*k]])

                pickle.dump(support_x_app, open("{}/{}/batch/supp_x_size_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
                pickle.dump(support_y_app, open("{}/{}/batch/supp_y_size_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
                pickle.dump(query_x_app, open("{}/{}/batch/query_x_size_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
                pickle.dump(query_y_app, open("{}/{}/batch/query_y_size_{}_{}.pkl".format(master_path, state, k, idx), "wb"))
                idx += 1


def save_per_object_content_tensor():
    master_path= "./ml"
    dataset_path = "movielens/ml-1m"
    rate_list = load_list("{}/m_rate.txt".format(dataset_path))
    genre_list = load_list("{}/m_genre.txt".format(dataset_path))
    ...
    # hashmap for item information
    movie_content = torch.zeros(3953, 10242).long()
    for idx, row in dataset.item_data.iterrows():
        m_info = item_converting(row, rate_list, genre_list, director_list, actor_list)
        movie_content[int(row['movie_id']),:] = m_info
    pickle.dump(movie_content, open("{}/movie_content.pkl".format(master_path), "wb"))
    # hashmap for user profile
    user_content = torch.zeros(6041, 4).long()
    for idx, row in dataset.user_data.iterrows():
        u_info = user_converting(row, gender_list, age_list, occupation_list, zipcode_list)
        user_content[int(row['user_id']),:] = u_info
    pickle.dump(user_content, open("{}/user_content.pkl".format(master_path), "wb"))


def generate_first_k_np(master_path, k):
    dataset_path = "movielens/ml-1m"
    for state in states:
        support_sets = []
        query_sets = []
        ...
        for _, user_id in tqdm(enumerate(dataset.keys())):
            ...
            for i in indices[:min(k, len(indices)-10)]:
                m_id = int(tmp_x[i])
                rating = tmp_y[i]
                support_task.append([u_id, m_id, rating])
            for i in indices[-10:]:
                m_id = int(tmp_x[i])
                rating = tmp_y[i]
                query_task.append([u_id, m_id, rating])
            support_sets.append(np.array(support_task))
            query_sets.append(np.array(query_task))
        pickle.dump(support_sets, open("{}/{}_supp_first_{}.pkl".format(master_path, state, k), "wb"))
        pickle.dump(query_sets, open("{}/{}_query_first_{}.pkl".format(master_path, state, k), "wb"))


def generate_batch_k_np(master_path, k):
    dataset_path = "movielens/ml-1m"
    for state in states:
        support_sets = []
        query_sets = []
        ...
        for _, user_id in tqdm(enumerate(dataset.keys())):
            ...
            for batch in range(seen_movie_len // (2*k)):
                support_task = []
                query_task = []
                for i in indices[batch*2*k:batch*2*k+k]:
                    m_id = int(tmp_x[i])
                    rating = tmp_y[i]
                    support_task.append([u_id, m_id, rating])
                for i in indices[batch*2*k+k:batch*2*k+2*k]:
                    m_id = int(tmp_x[i])
                    rating = tmp_y[i]
                    query_task.append([u_id, m_id, rating])
                support_sets.append(np.array(support_task))
                query_sets.append(np.array(query_task))
        pickle.dump(support_sets, open("{}/{}_supp_batch_{}.pkl".format(master_path, state, k), "wb"))
        pickle.dump(query_sets, open("{}/{}_query_batch_{}.pkl".format(master_path, state, k), "wb"))


def generate_regular_learning_data():
...
# save_per_object_content_tensor()
